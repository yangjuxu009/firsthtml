...
Calendar.prototype.draw = function () {
  ...
  this.form.selectTodayButton.onclick = function () {
    ...
    if (calendar.dateControl.value != null && calendar.dateControl.value != '' && calendar.dateControl.value.length > 0) {
      var currdate = today.format(calendar.date2StringPattern);
      if (calendar.dateControl.value.indexOf(currdate) != -1) {
        calendar.dateControl.value = calendar.dateControl.value;
        alert('已经选择了此日期！');
      } else {
        calendar.dateControl.value = calendar.dateControl.value + ',' + today.format(calendar.date2StringPattern);
      }
    } else {
      ...
      tds[i].onclick = function () {
        if (calendar.dateControl) {
          if (calendar.dateControl.value == null || calendar.dateControl.value == '' || calendar.dateControl.value.length == 0) {
            ...
            var currdate = new Date(calendar.date.getFullYear(),
              calendar.date.getMonth(),
              this.innerHTML).format(calendar.date2StringPattern);
            if (calendar.dateControl.value.indexOf(currdate) != -1) {
              calendar.dateControl.value = calendar.dateControl.value;
              ...
              calendar.dateControl.value = calendar.dateControl.value + ',' + new Date(calendar.date.getFullYear(),
                calendar.date.getMonth(),
                this.innerHTML).format(calendar.date2StringPattern);
            }
          }
